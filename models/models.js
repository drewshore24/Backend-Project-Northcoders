...
const selectArticles = (sort_by, order, topicvalue) => {
  const querysanatizedColumns = "SELECT * FROM articles"
 return db.query(querysanatizedColumns).then((columns) => {  
  const artcileObjects = columns.rows
  const sanatizedtopicValues = [];
  const sanatizedColumns = [];
  artcileObjects.forEach((article) => {
